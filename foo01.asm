...
main(int argc, char *argv[])
{
  ...
  if (argc != 2) {
  1a:	0f 84 9a 00 00 00    	je     ba <main+0xba>
    printf(2, "usage: %s n\n", argv[0]);
...
main(int argc, char *argv[])
{
  ...
  5d:	7d 21                	jge    80 <main+0x80>
    id = fork ();
  5f:	e8 b6 02 00 00       	call   31a <fork>
    if ( id < 0 ) {
  ...
  66:	89 c3                	mov    %eax,%ebx
    if ( id < 0 ) {
  68:	79 d6                	jns    40 <main+0x40>
      printf(1, "%d failed in fork!\n", getpid() );
  6a:	e8 33 03 00 00       	call   3a2 <getpid>
  6f:	51                   	push   %ecx
  70:	50                   	push   %eax
  71:	68 cd 07 00 00       	push   $0x7cd
  76:	6a 01                	push   $0x1
  ...
  exit();
  80:	e8 9d 02 00 00       	call   322 <exit>
      printf(1, "%d failed in fork!\n", getpid() );
      break;
    } else if ( id > 0 ) {  //parent
      printf(1, "Parent %d creating child  %d\n", getpid(), id );
  ...
  85:	e8 18 03 00 00       	call   3a2 <getpid>
  8a:	52                   	push   %edx
  8b:	50                   	push   %eax
  8c:	68 ff 07 00 00       	push   $0x7ff
  91:	6a 01                	push   $0x1
  ...
  9a:	83 c4 10             	add    $0x10,%esp
      for ( z = 0; z < 8000000.0; z += 0.001 )
  9d:	dd 05 18 08 00 00    	fldl   0x818
  a3:	90                   	nop
  a4:	8d 74 26 00          	lea    0x0(%esi,%eiz,1),%esi
  a8:	dc c1                	fadd   %st,%st(1)
  aa:	d9 05 20 08 00 00    	flds   0x820
  b0:	df ea                	fucomip %st(2),%st
  b2:	77 f4                	ja     a8 <main+0xa8>
  b4:	dd d8                	fstp   %st(0)
  b6:	dd d8                	fstp   %st(0)
  b8:	eb c6                	jmp    80 <main+0x80>
  ...
  bd:	ff 70 04             	pushl  0x4(%eax)
  c0:	e8 eb 01 00 00       	call   2b0 <atoi>
  for ( k = 0; k < n; k++ ) {
  ...
  for ( k = 0; k < n; k++ ) {
  cc:	0f 8f 67 ff ff ff    	jg     39 <main+0x39>
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 43f:	f7 f6                	div    %esi
 441:	0f b6 92 2c 08 00 00 	movzbl 0x82c(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 5b8:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
 5bb:	b8 24 08 00 00       	mov    $0x824,%eax
...
free(void *ap)
{
 ...
 697:	89 08                	mov    %ecx,(%eax)
  freep = p;
 699:	a3 d0 0a 00 00       	mov    %eax,0xad0
}
 69e:	5b                   	pop    %ebx
 69f:	5e                   	pop    %esi
 6a0:	5f                   	pop    %edi
 ...
 6ba:	03 53 fc             	add    -0x4(%ebx),%edx
    ...
    p->s.ptr = bp;
  freep = p;
 6bd:	a3 d0 0a 00 00       	mov    %eax,0xad0
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 794:	eb e9                	jmp    77f <malloc+0xaf>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 796:	c7 05 d0 0a 00 00 d4 	movl   $0xad4,0xad0
 79d:	0a 00 00 
 7a0:	c7 05 d4 0a 00 00 d4 	movl   $0xad4,0xad4
 7a7:	0a 00 00 
    base.s.size = 0;
 7aa:	b8 d4 0a 00 00       	mov    $0xad4,%eax
